...
\section*{Why are we learning R? And who names a program after a
letter?}\label{why-are-we-learning-r-and-who-names-a-program-after-a-letter}
\addcontentsline{toc}{section}{Why are we learning R? And who names a
program after a letter?}

\markright{Why are we learning R? And who names a program after a
letter?}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{chapters/images/R_logo.svg.png}}

}

\caption{fun fact : R was named because it was inspired by another
program called S, and R comes after S in the English alphabet.}
...
needs to be sometimes, but I promise you will learn! In fact, that's the
point of this class.

It's totally okay (and expected) for you to feel frustrated at times;
this is part of the learning process. So please embrace the ``I HAVE NO
IDEA WHAT I'M DOING'' dog meme energy as you embark on your R journey.

\marginnote{\begin{footnotesize}

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{chapters/images/r_meme_dog.png}}

}

\caption{look how happy the doggy looks while learning}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{R} is the powerful, free, and friendly computer program that
  we will use to analyze data in this class. This website is not super
  friendly - choose the operating system you have (Windows, MacOS, or
  Linux) and then download the ``latest release'' on the next page.
  \textbf{If you have a chromebook or iPad / tablet, you will need to
  use \href{https://rstudio.cloud/}{posit.cloud}.}
\item
  ...
  not 100\% necessary, but makes it a little easier to navigate R. You
  will need to install R first in order for RStudio to work. \emph{Mac
  Users : make sure you drag the program from the virtual disc into your
  Applications Folder to fully install the program.}
\end{enumerate}
...
\addcontentsline{toc}{subsection}{Navigating R}
...
\addcontentsline{toc}{subsubsection}{VIDEO : Navigating R}
...
\addcontentsline{toc}{subsubsection}{VIDEO : Navigating RStudio}
...
\href{https://commons.wikimedia.org/wiki/File:The_oldest_writing_in_the_world_-_The_Sumerian_Stone_Tablet.jpg}{\begin{center}
\pandocbounded{\includegraphics[keepaspectratio]{chapters/images/clipboard-4017464194.png}}
\end{center}
}

\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-important-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{To-Do List (due before our next lecture)}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, leftrule=.75mm, bottomrule=.15mm]

\begin{itemize}
\item
  Read this chapter and watch the videos.
\item
  Complete the practice exercises (check-ins) and try to create some
  variables in R.
\item
  Take Quiz 1 (on our course page).
\item
  Think about something you would want to learn about people (or
  non-human animals) if you were a researcher.
\end{itemize}
...
complexity. Indeed, historical records of the earliest written language
reveal that people have used some form of symbols and numbers to
represent - the Sumerian (modern day Iraq) stone tablet at the beginning
of this chapter was used to represent where locusts and caterpillars had
infested different plots of land.

Psychology uses this language to better understand differences in people
(or non-human animals), and other scientific disciplines focus on their
own domains; physicists seek to understand differences (and
similarities) in matter and energy, chemists seek to understand
differences (and similarities) in elements and compounds, botanists seek
to understand differences in plants, and economists seek to understand
money.
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.7781}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.2219}}@{}}
\toprule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
As an example, let's look to the stars. Astronomers have developed
knowledge about celestial bodies - gravity, orbits, mass (I know very
little about this). But I trust this science because astronomers are
able to use it to make very valid predictions about giant space rocks
and when they will come close to the earth. I can use this knowledge to
plan a stargazing trip, or plan to watch all the rich people leave earth
when the ``big one'' comes for the rest of us.

\marginnote{\begin{footnotesize}

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{chapters/images/clipboard-3451448147.png}}

}

\caption{You can see when scientists predict Halley's comet to come
closest to earth \href{https://theskylive.com/halley-tracker}{here}.
While this is fairly accurate, I've seen the exact prediction change
over the years - there is still some error in this prediction.}
...
Think about some predictions that you have made. What knowledge informed
the prediction? Did you use this prediction to influence your future
behavior in some way? What kinds of predictions do psychologists make?
We will chat more about these ideas in class :) but it's a core focus on
why psychologists use predictions.
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-caution-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Click Here to See The Answer}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-caution-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\section{Part 2 : Defining Variables in
R}\label{part-2-defining-variables-in-r}

Below are some videos, and R code, that review how to define a variable
in R. Practice creating your own script in RStudio to follow along;
there's value in typing this out yourself to get that muscle memory in
💪🤘.
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Prof.~Cat Has An Idea}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
Crisis}. There are two parts to the crisis:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Researchers tend not to seek to replicate their (or others') results.
\item
  Systematic attempts to replicate previous research suggests that the
  majority of results in psychology do not replicate\footnote{You can go
    deeper into some of the drama surrounding the replication project
    \hyperref[0]{here}. lemme know what you think on the discord thread
    for this week.}.
\end{enumerate}
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-important-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{To-Do List:}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\item
  ...
  \item
    ...
    understand these data (\textbf{Part 1 : Statistics})
  \item
    use R to define variables, import datasets, and navigate these
    datasets (\textbf{Part 2 : R})
  \item
    navigate the big business of academic research, and find research
    articles relevant to your interests (\textbf{Part 3 : Research
    Methods})
  \end{itemize}
\item
...
on the arrow ---\textgreater{}}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{What are the factors and levels in the example above?}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\NormalTok{haas[,}\DecValTok{3}\NormalTok{]}
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Answer Key : Indexing!}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-important-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
but first it will be important to learn how to identify the different
types of articles. There are a few different types of articles that
researchers write :
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-warning-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Academic Publishers are Predatory Capitalists.}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-warning-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
to a college library, or ``steal'' articles from predatory publishers
who don't pay authors using tools like sci-hub or annas-archive).
...
\item
  ...
  {[}\includegraphics[width=0.41667in,height=0.13542in]{index_files/mediabag/AD_4nXdWGVqSlFveQI-X.pdf}{]}that
...
recommended to \textbf{go to Graduate School,} where you can spend
...
In class this week, we will continue to get practice working with
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Activity : What's the median?}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Reasons for this bimodal distribution.}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\chapter{Part 3. Reading Research.}\label{part-3.-reading-research.}

This week, we will learn different methods of reading a research
article. To help support this journey, read the following two articles

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \href{https://web.stanford.edu/class/ee384m/Handouts/HowtoReadPaper.pdf}{How
  to read a paper.}. I like this article because it emphasizes that
  there are different layers to reading a paper. Use this paper to help
  you read the next one :)
\item
  \href{https://pmc.ncbi.nlm.nih.gov/articles/PMC7392212/}{Ten simple
  rules for reading a scientific paper.} This article comes from a
  biology perspective, but has a nice overview of the different parts to
  a research paper, and some of the broader questions to consider.
\end{enumerate}

Which article was more helpful or useful? Did the first paper help you
read the second paper at all? What do you like to read (for fun or
school?) Let us know on Discord :)
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-important-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{In This Chapter}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, leftrule=.75mm, bottomrule=.15mm]

\begin{itemize}
\item
  Define, discuss, and critique the idea of ``normality'' and
  ``deviance''.
\item
  Use R to calculate a z-score, which quantifies an individual's
  deviance from the mean.
\item
  Learn how scientists evaluate the quality of data, and see how this
  was used to debunk phrenology as racist.
\end{itemize}
...
\subsection{Example 1 : Thinking About
Distributions.}\label{example-1-thinking-about-distributions.}

Indeed, many of the variables that psychologists measure do appear
``Normal''. For example, let's look at one example ``normal''
distributions - a ``self-esteem'' variable. Note that it's not perfectly
normal\footnote{good not to hold data to unrealistic body images as well
  as people.}, but it's pretty close and representative of the kind of
``real world'' data that you might encounter.
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/4R_NormalPeople_files/figure-pdf/unnamed-chunk-2-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{``Random'' Reasons for Variation}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-warning-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Culture in Statistics : The Normal Curve}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-warning-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/4R_NormalPeople_files/figure-pdf/unnamed-chunk-6-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/4R_NormalPeople_files/figure-pdf/unnamed-chunk-9-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/4R_NormalPeople_files/figure-pdf/unnamed-chunk-12-1.pdf}}
...
\chapter{Part 3 : Good Data (Validity and
Reliability)}\label{part-3-good-data-validity-and-reliability}
...
\section{Validity and Its Specific
Forms}\label{validity-and-its-specific-forms}
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Expand To See Answers}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\item
  ...
  recent research}to test phrenology's theories. \# TLDR.
\end{itemize}

Developing a valid science of people is hard at best, and can be
problematic when you use it to rank people. There are lots of different
methods to measure what individuals are like, and for your final project
you probably will want to give people self-report surveys to keep things
simple for this very first study.

\chapter{Good Measures}\label{good-measures}

\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-important-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{In This Chapter}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, leftrule=.75mm, bottomrule=.15mm]

\begin{itemize}
\item
  Learn about the different methods and measures that psychologists use
  to collect data.
\item
  Learn about how (and why) \textbf{likert scales} are used to define
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/images/clipboard-2526453233.png}}

\chapter{Part 1 : Hard Data in a Soft
Science}\label{part-1-hard-data-in-a-soft-science}
...
\textbf{Examples} & \textbf{Informant Reports.} Informant reports are a
special form of surveys, where researchers ask friends, family, or
strangers to answer survey questions about another person. This is
technically observational data, since the people answering the surveys
are basing their judgments on their observations of the individual.
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/images/4_RAnotes.png}} \\
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.0290}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.9710}}@{}}
\toprule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\section{The Likert Scale}\label{the-likert-scale}

\subsection{Definition \& Theory}\label{definition-theory}
...
\subsection{Video Example : The Extraversion
...
\subsection{\texorpdfstring{\href{https://docs.google.com/forms/d/e/1FAIpQLSdR2QnugvaNWZUptvq8HUitM94pICmsR_17Wiy0LKgmORvZIQ/viewform?usp=sf_link}{Check-In
to Practice Reverse
Scoring}}{Check-In to Practice Reverse Scoring}}\label{check-in-to-practice-reverse-scoring}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/5R_GoodMeasures_files/figure-pdf/unnamed-chunk-8-1.pdf}}
...
\chapter{Likert Scale Quiz.}\label{likert-scale-quiz.}

Note : Creating likert scales takes practice. So I went ahead and
uploaded a video key for this quiz (as part of the quiz assignment). You
should try to take the quiz on your own - good to have a little bit of
struggle! - and use the video key as a guide to learn.
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-note-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{\href{https://docs.google.com/forms/d/e/1FAIpQLSev3Ot9gGd9QcHpEh5GhWifCAA7c_A-ycKT-7Zg5orC3dtGlg/viewform?usp=sf_link}{Check-In:
Intercepts and Slopes}}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
: Understanding \(R^2\)}}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-warning-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{RECAP : Uhhh\ldots can you re-explain Z-Scoring Professor???}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-warning-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\chapter{Linear Model : Categorical
IV}\label{linear-model-categorical-iv}

\chapter{Chapter Overview}\label{chapter-overview}

\begin{figure*}

\pandocbounded{\includegraphics[keepaspectratio]{chapters/images/clipboard-2948302145.png}}

\end{figure*}%

This week, we will learn how to define and interpret a linear model when
the predictor is categorical.

Often times, researchers want to group people into categories that
represent broad and important differences. For example researchers or
\href{https://en.wikipedia.org/wiki/Man_at_the_Crossroads}{artists}
looking to understand social class might divide individuals into groups
like capitalists and workers.

These categorical variables highlight ways that people differ - there
are real and important differences between people who own the means of
production, and people who do the work. However, categorical variables
can also exaggerate the differences between groups (e.g., both
capitalists and workers probably love their children in similar ways)
and can minimize individual differences within groups (e.g., workers can
differ in terms of their income, their race, their attitudes about
capitalism, etc.)
...
It's been a while since we focused on categorical variables, so let's
review some terms as we work through an example.

The specific groups of a categorical variable are called
\textbf{levels}, and the broad name for the groups is called a
\textbf{factor.} For example, in the ``MBA Business Student Data''
...
with just two levels : \textbf{female} and \textbf{male}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mba }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"\textasciitilde{}/Dropbox/!WHY STATS/Chapter Datasets/Hormone Data/hormone\_dataset.csv"}\NormalTok{, }\AttributeTok{stringsAsFactors =}\NormalTok{ T)}
\NormalTok{mba}\SpecialCharTok{$}\NormalTok{sex }\OtherTok{\textless{}{-}} \FunctionTok{as.factor}\NormalTok{(mba}\SpecialCharTok{$}\NormalTok{sex)}
\FunctionTok{levels}\NormalTok{(mba}\SpecialCharTok{$}\NormalTok{sex) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"male"}\NormalTok{, }\StringTok{"female"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\$\huge y\_i = a + b\_1 * X\_i + \epsilon\_i\$
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{What Professor Sees.}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\section{\texorpdfstring{\(R^2\) and the Linear Model with a Categorical
IV}{R\^{}2 and the Linear Model with a Categorical IV}}\label{r2-and-the-linear-model-with-a-categorical-iv}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{levels}\NormalTok{(mba}\SpecialCharTok{$}\NormalTok{sex)}
\end{Highlighting}
\end{Shaded}
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Answers to Personality and Longevity Example}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1496}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.6461}}@{}}
\toprule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
this question, and who is in the sample?}}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
systematic bias?}}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
the bias influence the results?)}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
would you want the t-value to be LARGE or SMALL?}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
-1.25010 -0.34770  0.00201  0.27432  1.41182 

Coefficients:
            Estimate Std. Error t value Pr(>|t|)    
(Intercept)  4.74631    0.61697   7.693 7.83e-12 ***
age         -0.05654    0.02216  -2.552   0.0122 *  
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

Residual standard error: 0.5217 on 106 degrees of freedom
  (14 observations deleted due to missingness)
Multiple R-squared:  0.05786,   Adjusted R-squared:  0.04898 
F-statistic:  6.51 on 1 and 106 DF,  p-value: 0.01215
\end{verbatim}
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Answers to Google Shade of Blue Experiment}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
to Model 3? From Model 2 to Model 3?}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Example Model}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{h }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"\textasciitilde{}/Dropbox/!GRADSTATS/Datasets/Hormone Data/hormone\_dataset.csv"}\NormalTok{)}
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{What Professor Observes (Think On Ur Own First?)}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, coltitle=black, opacitybacktitle=0.6, arc=.35mm, colframe=quarto-callout-tip-color-frame, colback=white, rightrule=.15mm, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{What Professor Thinks Went Wrong (Think On Ur Own First?)}, titlerule=0mm, toptitle=1mm, left=2mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, leftrule=.75mm, bottomrule=.15mm]
...
\subsection{Numeric Variables in R}\label{numeric-variables-in-r}
...
\section{Importing and Navigating
Datasets}\label{importing-and-navigating-datasets}
...
\section{Working with One Variable}\label{working-with-one-variable}

\subsection{Visualizing a Variable}\label{visualizing-a-variable}
...
\subsection{Descriptive Statistics for a
Variable}\label{descriptive-statistics-for-a-variable}
...
\subsection{Removing Outliers from a
Variable}\label{removing-outliers-from-a-variable}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.1577}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.8423}}@{}}
\toprule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{R Command} & \textbf{What It Does} \\
\texttt{dat\$variable{[}dat\$variable\ \textgreater{}\ \#{]}\ \textless{}-\ NA}
& One method for removing outliers - the code inside the bracket sets a
rule to identify possible outliers, and then the \textless- NA tells R
to remove these data. Make sure to change the operator (e.g.,
\textgreater{} or \textless{} or ==) depending on the type of outlier
you need to remove. \\
\texttt{dat\$variable{[}dat\$variable\ ==\ “baddata”{]}\ \textless{}-\ NA}
& The method adapted for excluding a specific string response from a
variable. \\
\end{longtable}

\subsection{Defininig a Likert Scale
Variable}\label{defininig-a-likert-scale-variable}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.1453}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.8547}}@{}}
\toprule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{R Command} & \textbf{What It Does} \\
\texttt{SCALE.df\ \textless{}-\ data.frame(dat\$item1,\ dat\$item2,\ dat\$item3)}
& Organize your items into a dataframe. Make sure that all items are
keyed in the same direction. If a variable is negatively-keyed (e.g., a
high response on the item means they are LOW on the variable) then you
will need to reverse score it by subtracting from range+1 (e.g., a 1-5
scale = 6 - item). \\
\texttt{alpha(SCALE.df)} & From the `psych' package; evaluates the alpha
reliability of your scale. The higher the alpha, the more reliable the
scale. Alpha increases as the items become more related to each other
and as the number of items increases. For scales with 2-3 items, an
alpha around .3-.5 is expected; for scales with 8 items an alpha over .8
is considered ``good''. \\
\texttt{dat\$SCALE\ \textless{}-\ rowMeans(SCALE.df)} & The final step;
averages all the items from your dataframe into a variable. \\
\end{longtable}

\section{Predictive Statisitcs}\label{predictive-statisitcs}

\subsection{Linear Models}\label{linear-models}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.1424}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.8576}}@{}}
\toprule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{R Command} & \textbf{What It Does} \\
\texttt{mod\ \textless{}-\ lm(DV\ \textasciitilde{}\ IV,\ data\ =\ dat)}
& defines a linear model, predicting the DV (a variable) from the IV,
and saves this model as an object. \\
\texttt{plot(DV\ \textasciitilde{}\ IV,\ data\ =\ dat)} & WHEN THE IV IS
CONTINUOUS : draws a scatterplot with individual DV scores on the y-axis
and IV scores on the x-axis \\
\texttt{abline(mod)} & draws the linear model on the above plot. \\
\texttt{summary(mod)\$r.squared} & the `easy' way to calculate R2 \\
\texttt{plot(mod)\ \#\#\ CAREFUL!} & WARNING : returns a series of
diagnostic plots for the regression analysis; keep hitting {[}enter{]}
to move to the next plot, or {[}escape{]} to get out of the plots. you
probably meant ot plot(DV \textasciitilde{} IV, data = dat) \\
\texttt{plotmeans(DV\ \textasciitilde{}\ IV,\ data\ =\ dat,\ connect\ =\ F)}
& WHEN IV IS CATEGORICAL - REQUIRES THE gplots LIBRARY : draws a plot of
a linear model with a categorical IV; the predicted value of each group
is a dot, with standard error bars drawn above and below to illustrate
sampling error. connect = F removes the line that illustrates the
slope. \\
\end{longtable}

\subsection{Inferential Statistics
(NHST)}\label{inferential-statistics-nhst}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.2133}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.7867}}@{}}
\toprule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{R Command} & \textbf{What It Does} \\
summary(mod) & returns the null hypothesis significance testing
results \\
\end{longtable}
