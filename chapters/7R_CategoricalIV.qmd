# The Linear Model : Categorical IV

::: column-screen
![](images/clipboard-2948302145.png)
:::
...
Often times, researchers want to group people into categories that represent broad and important differences. For example researchers or [artists](https://en.wikipedia.org/wiki/Man_at_the_Crossroads) looking to understand social class might divide individuals into groups like capitalists and workers.
...
It’s been a few weeks since we focused on categorical variables, so let's review some terms as we work through an example.

The specific groups of a categorical variable are called **levels**, and the broad name for the groups is called a **factor.** For example, in the “MBA Business Student Data” ([hormone_data.csv](https://www.dropbox.com/scl/fi/b2vq1yi5azwb5u8j7xe74/hormone_data.csv?rlkey=lv77ppuseg5cmqsy99pal55ct&dl=0)) researchers measure the variable **sex** as a categorical factor, with just two levels : **female** and **male**.
...
levels(mba$sex) <- c("male", "female")
mba$sex <- relevel(mba$sex, ref = "female")
```
...
abline(h = mean(mba$NPI, na.rm = T), lwd = 0)
```
...
R defaults to alphabetical order - because F (for Female) comes earlier in the alphabet than M (for Male), when X = 0, we will be referring to the Females in the dataset, and when X = 1, we will be referring to the Males in the dataset.[^7r_categoricaliv-1]

[^7r_categoricaliv-1]: It’s up to you which group will be assigned the value 0 and which group will be assigned the value 1, and later we will learn how to swap this around and why it might matter.
...
One method I like is the plotmeans() function. This function comes from the gplots package. You will need to install this package once, then load it from the library each time you start R.[^7r_categoricaliv-2]

[^7r_categoricaliv-2]: install.packages(“gplots”) to install the package, then library(gplots) to load.

Below is what it looks like when I install the gplots package (remember you only need to do this once) and then load the gplots library.[^7r_categoricaliv-3]

[^7r_categoricaliv-3]: The warning message is telling me that the gplots and the stats libaries both have a function called lowess. R is “masking” this function from stats, which means if I refer to the lowess function , R will think I mean the one that comes from the gplots package. If I want to use the lowess from the stats library, I will need to manually tell R to do this using stats::lowess(). We won’t use the lowess function in this class.
...
## $R^2$ and the Linear Model with a Categorical IV
...
Okay, last thing. Remember how I write that the order of the levels (female = 0, male = 1) is arbitrary, and R defaults to alphabetical order? Sometimes you want to change the order around - in this example, make male = 0 and female = 1.[^7r_categoricaliv-4]

[^7r_categoricaliv-4]: It’s not important to do releveling in this example. However, sometimes one group is a clear reference group that should be assigned as the intercept. For example, if one group is the “default” experience, and you want to see how the other group changes that default. We’ll chat more about this next week.
...
## Practice Quiz : Defining a Linear Model with a Categorical IV
...
<iframe src="https://www.loom.com/embed/909ad220c4d44d39886cd805eccbcb3f" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;">
...
coef(test.mod)
plotmeans(test ~ sex, data = mba, connect = F)
```
...
<iframe src="https://www.loom.com/embed/3e1e386827354ab2a5ac5a0731f46b66" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;">
...
<iframe src="https://www.loom.com/embed/d68a666340504fee871228d4d188ce54" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;">
...
Consider the example below, from a research study that reported a relationship between how much explicit prejudice against indigenous groups people reported, and people's (positive) attitudes toward a Native American mascot.
