...
Below are data from 2012[^9r_samplingerror-1], when Barack Obama was running against Mitt Romney in the US Presidential Election. Look over the data, and think about the answers to the three questions to evaluate sampling bias.

[^9r_samplingerror-1]: source : <https://fivethirtyeight.com/features/obamas-lead-looks-stronger-in-polls-that-include-cellphones/>
...
What we've done - through the power of triangles (the strongest shape, both physically and psychologically) - is generate a dataset[^9r_samplingerror-2] that we will consider to be a population of data.

[^9r_samplingerror-2]: You can [access the fake dataset here](https://www.dropbox.com/scl/fi/cvcwam7xe202nuocbfmbp/brainwavebabydata.csv?rlkey=5hi2xknvv83cgydurts31fvlw&dl=0) if you want to follow along in R, but this is not necessary and I think reading these notes should suffice.
...
We can illustrate this in R. R has a built-in function called sample(), which takes a random sample[^9r_samplingerror-3] of whatever numbers you give it. For example, here’s a fun game - think of a number between 1 and 10. Then ask R to sample() a random number between 1 and 10 and see if you are vibing with R. I am thinking of the number FOUR.

[^9r_samplingerror-3]: There are different ways that computers can take “random samples”, each with different merits. The default method R uses is “"Mersenne-Twister" From Matsumoto and Nishimura (1998). This stuff goes way beyond my pay grade; I imagine there are a few explanations on the internet. Let me know if you find a good one!

    \

In the code below, I’m asking R to define the numbers 1 through 10, and then draw ONE number from this set.[^9r_samplingerror-4]

[^9r_samplingerror-4]: Note that when I use the `set.seed()` function before the `sample()` function, I'm telling R to "fix" the random number generator it uses. This gives me a consistent result time it runs the "random" `sample()` function. And if you also run `set.seed(42)` before you run `sample(1:10, 1)` you should also get the same answer of 1. You can specify any number within `set.seed()`, and only want to use this function when you want others to get the same answer as you (like when sharing code). Most of the time, we don't want to use this function because we want things to be "random". Let me know if this is confusing! I can try to clarify.
...
-   **the effect we found in our study is not due to chance.** there is still a chance that we are making a mistake in our estimate of sampling error. and even a small p-value of .000001 means there is a chance that we observed this slope due to random sampling error[^9r_samplingerror-7].
...
Positive and Negative 1.96 are the values of the normal distribution that contain 95% of the scores[^9r_samplingerror-8]. Multiplying this value by the estiamted standard error of the slope (which describes how much the slope might vary due to random sampling error) translates these boundaries into the same units as your slope. You can visualize the 95% Confidence Interval below.

[^9r_samplingerror-8]: The exact range of the 95% confidence interval will not always be within ±1.96 standard errors of the slope - that’s the theoretical range based on a normal distribution. In practice, the range will depend on certain features of your sample and model (much like the t-distribution we discussed), but will approach 1.96 as your sample size increases; and if you want to be more conservative, you can round up to 2\*SEb as long as your sample size is \> 30.
