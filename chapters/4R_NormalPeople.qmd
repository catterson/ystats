...
Indeed, many of the variables that psychologists measure do appear "Normal". For example, let’s look at one example "normal" distributions - a "self-esteem" variable. Note that it's not perfectly normal[^4r_normalpeople-5], but it's pretty close and representative of the kind of "real world" data that you might encounter.

[^4r_normalpeople-5]: good not to hold data to unrealistic body images as well as people.
...
Below is another example of a distribution - this one is not random, but is skewed (pop quiz : is it positively or negatively skewed? See here for answer[^4r_normalpeople-6].)

[^4r_normalpeople-6]: It is negatively skewed, since the tail is on the negative side of the distribution.
...
[^4r_normalpeople-7]: see Rose's END OF AVERAGE (2016) or, for a more critical approach, Chapman's (2023) EMPIRE OF NORMALITY.
...
# Part 3 : Good Data (Validity and Reliability)
