...
For example, let’s look at one example "normal" distributions - the "self-esteem" variable we saw in Lecture 2. Note that it's not perfectly normal[^5r_goodscience-5], but it's pretty close and representative of the kind of "real world" data that you might encounter.
...
# Part 2. Z-Scoring and Outliers in R

The Z-Score is a linear transformation, that calculates the distance from the mean, in units of standard deviation.

Watch the video below to get a sense of what this transformation does, and why we should care.

{{< video https://youtu.be/wPX4tlDLXjE >}}

### In R : Z-Scores

#### Z-Scoring Individual Scores : Age

If we look at the variable age from the self-esteem dataset, we see some issues.

```{r}
hist(d$age)
```

This looks odd. R says this is a histogram, but it doesn't look like the histograms we've seen before for a few reasons :
...
In this code, I'm using indexing \[\] to ask R to find me the rows from the dataset d that contain ages that are greater than 120 years old AND ages that are not na values (the ! when used in coding means "not"); the comma with nothing after it inside the bracket tells R to find me all the columns from the d dataset, which I'm indexing.
...
But there are still some folks who are ... very old. Older than 80 years old. Which is amazing if true.
...
So, these elders exist. And maybe they are real. But should we include them?

#### Z-Scores "By Hand"

If we try to use the scale() function described above for z-scoring this group of elders, it doesn't look like anyone is too far away from the average in terms of their standard deviation.

```{r}
scale(elders)
```

Yet I would expect the person who is 118 to be much more than 2 standard deviations away from our new mean...so the scale function doesn't work in this case, because
...
Okay,now it seems like ALL of these elders (people older than 80 years old) are very different from the average. They are all more than 4 times the standard deviation. This gives me more confidence that I could remove them from the dataset.

But should I remove them from the dataset just because they are different?
